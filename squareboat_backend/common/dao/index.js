class BaseDao {
  ...
  findAndPopulate(query, projection , page , limit , select , _sort) {
    return this.Model.find(query, projection).sort(_sort).skip(page * limit)
    .limit(limit)
    .populate({ path: "userId" , select}).lean().exec();
  }
